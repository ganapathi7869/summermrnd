...
int set(char *str, struct node mat[10][10]){
	...
	char *hold, *oper1 = (char *)malloc(sizeof(char) * 5);
	while (str[ind]<'a' || str[ind]>'j')  ind++;
	c = str[ind++] - 'a';
	r = str[ind++] - '1';
	if (str[ind++] != '=')  return 1;
	str[ind - 1] = '\0';
	if (eval(&str[ind],mat, &str[ind - 3])==-1){
...
//	return number;
//}
int get(char *str, struct node mat[10][10]){
	int r, c;
	int ind = 0;
	while (str[ind]<'a' || str[ind]>'j')  ind++;
	c = str[ind++] - 'a';
...
int import(char *str, struct node mat[10][10],char *storefilename){
	FILE *fptr;
	int ind = 0, bufind = 0, csvpres = 0;
	char buf[50];
	while (str[ind]<'a' || str[ind]>'j')  ind++;
...
int main(){
	...
	while (1){
		if (!strncmp(inp, "set", 3)){
			...
			import(&inp[6], mat,storefilename);
		}
